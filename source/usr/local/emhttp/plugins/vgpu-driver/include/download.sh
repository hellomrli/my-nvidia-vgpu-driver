...
#
# LOCAL-FIRST behaviour: a driver package placed manually in
#   /boot/config/plugins/vgpu-driver/packages/<kernel-major>/
...
DL_URL="https://github.com/hellomrli/my-vgpu-driver/releases/download/${KERNEL_V}"
API_URL="https://api.github.com/repos/hellomrli/my-vgpu-driver/releases/tags/${KERNEL_V}"
...
if [ -n "${LOCAL_PKG}" ] && [ "${WANT}" = "latest" ]; then
  if md5_ok "${LOCAL_PKG}"; then
    echo "-------Using local driver package $(basename "${LOCAL_PKG}") (checksum OK)-------"
    exit 0
  else
    echo "-----WARNING: local package $(basename "${LOCAL_PKG}") failed checksum, re-downloading-----"
  fi
fi
